...
class MiddlewareModule extends AbstractModule {
   ...
   * @param {number} options.maxFileSize Maximum file size allowed by upload
   * @param {string} options.uploadDir Directory file upload should be stored
   ...
   */
  fileUploadParser(expectedFileTypes, options = { promisify: false }) {
    const fOpts = {
      maxFileSize: options.maxFileSize || getConfig('fileUploadMaxFileSize'),
      multiples: true,
      uploadDir: options.uploadDir || this.getConfig('uploadTempDir')
    };
   ...
   * @return {Function} The Express handler
   */
  urlUploadParser(expectedFileTypes, options = { promisify: false }) {
    if(!Array.isArray(expectedFileTypes)) {
      expectedFileTypes = [expectedFileTypes];
    }
    return async (req, res, next) => {
      return new Promise(async (resolve, reject) => {
        ...
        const outputDir = this.getConfig('uploadTempDir');
