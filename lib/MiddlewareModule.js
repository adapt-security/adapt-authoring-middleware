    ...
    return (req, res, next) => {
      return new Promise(async (resolve, reject) => {
        ...
        formidable(options).parse(req, async (error, fields, files) => {
          if(error) {
            if(error.code === 1009) return next(App.instance.errors.FILE_EXCEEDS_MAX_SIZE.setData({ maxSize: options.maxFileSize }));
            return next(error);
          }
          // covert fields back from arrays and add to body
          Object.keys(fields).forEach(k => req.body[k] = fields[k][0]);
      ...
      return new Promise(async (resolve, reject) => {
        ...
        try {
          ...
          if(e.code === 'ERR_INVALID_URL' || e.response.status === 404) {
            e = this.app.errors.INVALID_ASSET_URL.setData({ url: req.body.url });
          }
          return next(e);
        }
